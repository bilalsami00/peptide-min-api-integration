...
//     goal: data.goal,
//     id: data.clientId,
//     databaseId: data.databaseId, // STORE REAL DB ID
//     isFDAApproved: data.isFDAApproved,
//     dateValue: dayjs(data.date).format("YYYY-MM-DD"),
//   };

//   setEventsByDate(prev => {
//     const newState = { ...prev };
    
//     // For updates, remove old version
//     if (data.type === "update") {
//       for (const date in newState) {
//         newState[date] = newState[date].filter(e => e.id !== data.clientId);
//         if (newState[date].length === 0) delete newState[date];
//       }
//     }

//     // Add new/updated event
//     if (newState[data.date]) {
//       newState[data.date] = [...newState[data.date], newEvent];
//     } else {
//       newState[data.date] = [newEvent];
//     }
