...
//   try {
//     await dosageService.deletePeptideDosage(deletingEvent.id);

//     // Update local state to remove deleted item
//     setEventsByDate((prev) => {
//       const newState = { ...prev };
//       const deleteDate = deletingEvent.date;

//       if (deleteDate && newState[deleteDate]) {
//         newState[deleteDate] = newState[deleteDate].filter(e => e.id !== deletingEvent.id);
//         if (newState[deleteDate].length === 0) delete newState[deleteDate];
//       }

//       return newState;
//     });

//     setDeletingEvent(null); // Close modal
//   } catch (error) {
...
const CalendarPage: React.FC = () => {
  ...
  // ←––––– MONTHLY FETCH: runs every time `currentDate` (the visible month) changes
useEffect(() => {
  const fetchMonthData = async () => {
    const start = currentDate.startOf("month");
    const end   = currentDate.endOf("month");

    // build a list of YYYY-MM-DD strings for each day in the month
    const dates: string[] = [];
    for (
      let d = start.clone();
    ...
    await Promise.all(
      dates.map(async (dateStr) => {
        try {
          const resp     = await dosageService.getPeptideDosageByDate(dateStr);
          ...
          //   databaseId:    item.id,
          //   date:          item.date,
          //   dateValue:     dayjs(item.date).format("YYYY-MM-DD"),
          //   peptide: {
          //     id:   item.peptide_id,
  ...
  // };
  const handleDeleteEvent = async () => {
    if (!deletingEvent || !deletingEvent.databaseId) return;

    try {
      await dosageService.deletePeptideDosage(deletingEvent.databaseId);
      ...
      setEventsByDate((prev) => {
        ...
        if (deleteDate && newState[deleteDate]) {
          newState[deleteDate] = newState[deleteDate].filter(
            (e) => e.databaseId !== deletingEvent.databaseId
          );
          if (newState[deleteDate].length === 0) delete newState[deleteDate];
        }
  ...
  // };
  // const setupEditForm = (event: CalendarEvent) => {
  //   console.log("Editing event passed to setupEditForm:", event);

  //   setEditingEvent({
  //     ...event,
  //     // databaseId: event.databaseId ?? Number(event.id), // Only if id is numeric
  //     databaseId: event.databaseId ?? (!isNaN(Number(event.id)) ? Number(event.id) : undefined)
